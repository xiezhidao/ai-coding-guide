...
> 📚 **系列导航**：上一篇 [13 · 提示词（Prompt）写法](13-prompting.md) 教你「话该怎么说」——把模糊需求拆成 Codex 听得懂的精确指令。这一篇换个角度落地：日常 80% 的活儿就那四类，**每一类我给你一套能直接照抄的流程**，套进去填空就能用。
...
> Codex 在「能验证自己工作」的时候，产出质量明显更高。给它复现步骤、验证办法、跑 lint 和测试的指令——它就有据可依，而不是「看起来对」就交差。
...
![四类日常工作流](assets/14-workflows/four-workflows@2x.png)
...
这里要先讲一个 Codex 和 Claude Code 不一样的关键点，关系到你这三层问题怎么提：

> **IDE 扩展会自动把你打开的文件、选中的代码当上下文喂进去；但在 CLI 里，你通常得用 `@` 显式点出文件路径（或用 `/mention` 附一个具体文件）。**
...
**预期**：Codex 会先复现、再告诉你根因——**空列表时 `len(numbers)` 是 0，除以 0 就崩了**；然后给一个 diff（比如空列表时返回 0、或抛一个更清楚的异常），停下来等你批准；批准后它还会新建一个测试文件（像 `test_calc.py`），里面有一条专门测空列表的用例。
