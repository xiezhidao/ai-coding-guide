...
这一篇就把「派谁去」这件事讲透：**Codex 现在有哪几个模型、一个比换模型更立竿见影的旋钮（推理强度）、以及一张我自己天天在用的「场景 → 模型 + 强度」对照表。**
...
| 模型 | 定位 | 最适合 | 速度 / 成本 |
| --- | --- | --- | --- |
| **`gpt-5.5`** | 旗舰、默认推荐 | 复杂编码、计算机操作、知识工作、研究类工作流 | 较慢 / 最高 |
| **`gpt-5.4-mini`** | 轻量、快而省 | 轻量编码任务、给子代理用 | 快 / 低 |
| **`gpt-5.3-codex-spark`** | 即时型（研究预览） | 近乎实时的高频编码迭代（偏纯文本、能力较弱） | 极快 / 低 |
...
![模型档位怎么挑](assets/30-models/model-tiers@2x.png)
...
| `low` | 稍作分析 | 简单改动、快速迭代 |
| `medium` | 速度与深度平衡（常见默认档） | 日常大多数编码 |
| `high` | 深度推理 | 复杂重构、难缠 bug |
| `xhigh` | 顶格推理（取决于模型是否支持） | 最硬的架构与调试 |

注意两点：**`medium` 一般是默认起点**（具体默认值以你本地版本为准）；**`xhigh` 不是所有模型都吃**，能不能用看模型支持。
...
我的真实习惯是：**默认配置就是「旗舰 + `medium`」，绝大多数活儿不用动；只有进入「啃硬骨头」或「批量跑脏活」这两种模式时，我才会手动切一下。** 把切换当成「针对特定任务的临时动作」，而不是每次都纠结，你才不会被选择拖累。
