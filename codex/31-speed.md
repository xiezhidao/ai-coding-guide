...
先给你看我自己的一笔账。今年 3 月我做一个 Python 小工具，有个功能让我跟 Codex 来回折腾了整整一下午——我后来翻 `/status` 的会话记录数了数，光这一个需求就跑了 **11 轮**，期间它两次改错文件、一次把我没让它碰的配置也顺手「优化」了。复盘的时候我才反应过来：不是 `gpt-5.5` 不行，是我第一句话就只甩了一句「帮我加个导出功能」，剩下十轮全在补我本该一次说清的上下文。

**说句实话，AI 编码里最大的慢，从来不是模型转圈那几秒，是返工。** 一次没说清，它走一条岔路，你再拉回来，来回三趟，半小时没了。模型再快，也快不过「一次做对」省下来的那些轮次。
...
- 一个能立刻照做的动手练习：把一句含糊需求改写成三段式，亲眼看返工率怎么降

> ⚠️ 本篇命令、配置键、默认行为以 Codex 官方文档为准；模型名、信用点倍率、`/fast` 等会随版本变，**以你本地 `codex --help` 和官方文档实际为准**。
...
AI 编码一模一样。**Codex 走一条岔路、改错一个文件、领会错你的意图，你拉回来重说一遍——这一来一回的代价，远超它多想几秒。**
...
> 💡 一句话总结：AI 编码最大的慢是返工，提速的第一性原理是「一次做对」，而不是「跑得更快」。
...
我现在的习惯很死板：**这个需求做完了，下个不相关的需求，我直接新开会话。** CLI 里相关的几个命令很顺手：
...
> 上下文该「准」不该「多」。同一个问题就待在同一个会话里，保住推理链条；活儿真分叉了，才 `/fork` 或新开。
...
Codex 有个**快速模式（Fast mode）**，能把支持的模型提速约 1.5 倍，代价是信用点（credit）消耗更高。按官方说明，它目前支持 GPT-5.5 和 GPT-5.4，GPT-5.5 用快速模式按标准速率的 2.5 倍扣信用点，GPT-5.4 按 2 倍。
