...
├── (recent inputs)    # 近期输入
└── (supporting evidence from prior threads)  # 过往会话的佐证
```
...
想长期、全局地定规矩，写进 `~/.codex/config.toml` 的 `[memories]` 段。下面这几个是真用得上的（**默认值以官方配置参考为准，可能随版本变化**）：
...
**二是「会不会变 / 能不能复用」。** 一次性的、马上要改的、「先这么着」的，没必要（端口号、临时变量、这次的调试状态）；下次、下下次还用得上的（技术栈、项目惯例、踩过的坑），才是它该攒的。
...
**预期**：Codex 正常响应、照做。**但注意——这条不会立刻落进记忆文件。** 回想第 02 节：记忆是后台异步生成的，得等这个会话**空闲足够久**（官方默认 `min_rollout_idle_hours` 是 6 小时，以官方为准）、Codex 确认你不是还在干，才在后台总结。所以**别指望你下一句就能在文件里看到它**——这正是开头我同事踩的那个坑，亲手感受一下。
