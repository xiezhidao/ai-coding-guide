...
- **CLI（命令行）**：跑在终端里的编码代理，**三大平台全支持**，也是开发者最常用的方式。
...
装好打开后，左下角设置-常规-语言，自识别/切换语言之后，（右边栏和底部栏是通过右上角图标打开的）界面大致长这样：

![Codex 桌面 App 主界面：左侧导航、中间任务输入、连接卡片](assets/03-install/app-ui.png)
...
| **Homebrew**（Mac） | `brew install --cask codex`    | 装过 Homebrew | 已经重度用 brew 管软件的人，一般比官方更新安装延迟一两天，因为官方需要维护团队审核，但个人感觉这样反而更好，不易自己踩坑新版本 |
...
这是开头我朋友踩的那个坑：**在远程服务器、headless 环境、或本机网络挡了 localhost 回调时，浏览器登录这条路走不通**——要么浏览器开在另一台机器上，要么 OAuth 回调回不来。官方首选解法是**设备码登录**：

> 在交互登录界面选 **Sign in with Device Code**，或直接运行 `codex login --device-auth`（测试版功能，beta）；它给你一个链接和一次性验证码，你在**任意能上网的浏览器**打开链接、输码，就登上了。
