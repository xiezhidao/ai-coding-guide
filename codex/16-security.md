...
答案不是「模型很乖」，而是**两道由程序和操作系统强制执行的边界**——这俩你在第 02、15 篇已经认过脸了，这里只把它们摆到「安全」这个视角下重新看一眼。
...
![安全分层防护](assets/16-security/defense-layers@2x.png)
...
注意第一道「网络默认关」——**这是 Codex 防提示注入最朴素也最硬的一招**。攻击者再会写话术，命令根本联不上网，私钥也发不出去。这跟 Claude Code 默认拦 `curl` 黑名单是两种思路：Codex 干脆从源头掐了整个网络。
...
先说「读」这一关。Codex 默认工作区只含**当前目录和 `/tmp` 这类临时目录**（官方让你用 `/status` 查当前工作区到底包含哪些目录）。也就是说，**你 home 目录下的 `~/.ssh/`、`~/.aws/` 这些，默认不在 Codex 的工作区里**——它日常够不着。
...
那「全程不问」的危险模式具体长啥样？就这两个，**新手阶段当它们不存在**：
...
但这名字底下其实是**两个不同的东西**，新手最容易混。先用一句话钉死区别：
...
- [ ] 默认就用官方推荐档：`workspace-write` + `on-request`（版本控制目录），别一上来就放开
