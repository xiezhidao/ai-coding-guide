...
![子代理：并行分发、各自上下文、只回摘要](assets/21-subagents/subagent-isolation@2x.png)
...
```mermaid
flowchart TD
    A["一个任务来了"] --> B{"会刷大量中间输出?<br/>几块互不依赖?"}
    B -->|"否：小改 / 强依赖"| C["留在主线<br/>自己干更快更省"]
    B -->|"是"| D["你开口：拆成并行 agent"]
    D --> E["agent 1 探路"]
    D --> F["agent 2 跑测试"]
    D --> G["agent 3 翻文档"]
    E --> H["主线等齐三份<br/>汇总成一份结论"]
    F --> H
    G --> H
```
