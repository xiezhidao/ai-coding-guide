...
> 💡 **一句话总结**：电脑操控碰整个桌面，门槛最高——**地区上 EEA / 英国 / 瑞士不支持、Linux 没有**；macOS 装插件后给「屏幕录制 + 辅助功能」，Windows 会**抢你前台**；系统权限和应用审批是两码事，文件 / shell 照旧走沙箱；锁屏使用是 macOS 专属、默认关、刻意做窄，新手先别碰。
...
flowchart TD
    A["任务要碰界面?"] --> B{"哪种界面?"}
    B -->|"本地网页 / 公开页"| C["内置浏览器<br/>@Browser"]
    B -->|"要登录态的网站"| D["Chrome 扩展<br/>@Chrome"]
    B -->|"原生 App / 跨应用 / 没接口"| E["电脑操控<br/>@Computer"]
    C --> F["最轻最快<br/>不碰登录态"]
    D --> G["用你 Chrome 登录态<br/>默认逐站先问"]
    E --> H["碰整个桌面<br/>地区/平台有限制"]
```
