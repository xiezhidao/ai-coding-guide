...
说个我去年冬天干的蠢事。那会儿我刚把 Codex 配熟，图省事在 `~/.codex/config.toml` 里把 `sandbox_mode` 写死成了 `danger-full-access`，理由是「弹窗烦」。结果某天我在一个**没初始化 Git 的临时目录**里让它「清一下没用的文件」，它真就在我整个家目录里翻了起来——因为完全访问模式下根本没有「工作区」这道圈拦着它。我手忙脚乱按 `Esc` 打断，那一刻后背是凉的。
...
flowchart TD
    ...
    B -->|圈内| C[直接做<br/>不打扰你]
    B -->|要出圈| D{审批策略说咋办?}
    D -->|on-request / untrusted| E[停下来问你]
    D -->|never| F[不问 直接做]
    E -->|你同意| F
    E -->|你拒绝| G[放弃这步]
```
