...
**类比：一个带很多工位的开放式工作间。** CLI 像你一个人坐在一张桌子前埋头干，干完一件再下一件；桌面 App 则像你当了个小组长，眼前一排工位，每个工位坐一个 Codex 在干不同的活——这边在修 bug、那边在补测试、角落那个在跑长任务——你站在中间扫一眼就知道谁干到哪了，谁卡住了喊你拍板。**「同时盯住一排活、随时切过去插手」才是桌面 App 的本命**。
...
中间这一大块黑色区域是核心：上半是 Codex 的对话流（它读了啥、改了啥、跑了啥命令一路滚下来），底部是输入框。输入框这一排藏着几个关键开关，发指令前先扫一眼：
...
![桌面 App 五区布局（示意，真机以官方为准）](assets/07-desktop-app/five-zones@2x.png)
...
- **直接提交、推送、开 PR**：本地和 worktree 的任务，都能在 App 里直接 commit、push、建 Pull Request。
