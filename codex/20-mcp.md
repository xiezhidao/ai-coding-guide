...
这张图在说：**Codex 本地的本能只够得着左边的文件和命令**；中间的 MCP 就像个 USB hub，一头插进 Codex，另一头把右边它本来够不着的一堆外部 server——GitHub、数据库、网页搜索、Figma 这类第三方 API——统一接了进来。
...
server 加进来 ≠ 它的所有工具都得无脑放开。**Codex 在 `config.toml` 里给了一组字段，让你对单个 server 精细收口**——哪些工具能用、超时多久、调用要不要先问你。这些都是手写配置才能玩的细活，新手第一遍可以略过，有需求了再回来。
...
**预期**：列表里出现 `context7`。**看到它在 = 配置生效、server 挂上了。** 如果没出现或报错，多半是 Node 没装好、或网络没通——按上面那条前提排查。
...
**预期**：Codex 会**调用 Context7 的工具去拉那个库的最新文档**——按你的审批设置，**第一次调用这个 server 的工具时，它通常会停下来请求你批准**（这正是第 04、05 节讲的工具审批），批准它。然后它返回的是从文档 server 实时查来的写法，而不是它记忆里的老版本。**你能在它的执行过程里看到这一步走了 Context7**，这是你确认「答案真从外部文档来、不是模型瞎诌」的依据。
