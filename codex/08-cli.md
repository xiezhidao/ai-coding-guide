...
> 📚 **系列导航**：上一篇〔07 桌面 App 全景〕带你把图形界面那套点法摸熟了——看 diff、并排跑任务、可视化审批。这一篇调头钻进终端，搞懂 `codex` 这条命令到底怎么用：命令结构怎么拆、交互界面长啥样、常用选项和斜杠命令各管什么。下一篇〔09 IDE 扩展（VS Code 等）〕再把它塞进编辑器里。
...
**预期**：Codex 打印出当前会话的概况——用的哪个模型、审批策略是啥、有哪些可写目录、上下文还剩多少。**心里先有个底，知道自己现在是什么配置。也可以直接使用社区的 token-tracker 工具显示如图**

![token-tracker 显示用量：项目名、token 剩余、上下文占比、当前模型](assets/08-cli/tui-output.png)

显示数据：项目名称/5 小时 token 剩余量/一周使用剩余量/上下文占比/当前模型
