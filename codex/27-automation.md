...
都说「AI 编码工具最大的价值是陪你结对编程，边写边聊」——但说句实话，这话只说对了一半。
...
这里得先认一个词：`codex exec`。它是 Codex 的**非交互模式（non-interactive mode）**——不开那个全屏的交互界面，一句提示词丢进去、跑完吐结果就退出，专为脚本和 CI 设计。第 08 篇我们点过它「堂食 vs 外卖」那个比方，下一篇〔28〕会把它单独讲透。**这一篇你只要知道：GitHub Action 的底层，跑的就是 `codex exec`**——action 只是把「装 CLI、配认证、跑 exec」这一串在云端打包好了。
