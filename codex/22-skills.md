...
> ⚠️ 这里有个**新手最容易被老教程带歪的坑**：有些非官方资料里 frontmatter 写了个 `trigger:` 字段（说是「触发关键词」），还把 Skill 放进 `~/.codex/skills/`。**官方文档里没有 `trigger` 这个字段**，触发靠的是 `description` 的语义匹配（下文细说）；目录也不在 `~/.codex/skills`，而在 `.agents/skills` 那一套（第 04 节讲）。照老教程写，轻则不生效，重则你怎么调都调不出来。**一切以官方为准。**
...
```mermaid
flowchart TD
    Start([Codex 启动会话]) --> List[初始技能清单<br/>每个 Skill 只占<br/>名称 + 描述 + 路径]
    List --> Ask[你发来一个任务]
    Ask --> Match{任务对得上<br/>哪个描述？}
    Match -->|对上 Skill A| Load[只加载 A 的<br/>完整 SKILL.md 正文]
    Match -->|都没对上| Keep[谁的正文都不加载<br/>清单照旧只占一行]
    Load --> Run[Codex 照 A 的说明干活]
```

左边是常态——所有 Skill 在上下文里**各只占一句描述**，工作台还很空；你发来任务后，只有被命中的那个 Skill 的完整正文被加载进来，其余的仍然只是一行。一眼看出「省」在哪。

![Skills 渐进式披露两阶段](assets/22-skills/progressive-disclosure@2x.png)

这张图把两个阶段并排摆开：左边阶段一，启动时每个 Skill 只占「名称 + 一句描述 + 文件路径」，初始清单字符上限约为上下文的 2%，正文一概不读；右边阶段二，只有被任务命中 `description` 的那一个 Skill 才把完整 `SKILL.md`（含正文指令）展开读入，其余照旧躺在清单里只占一行——省上下文的秘密，全在这「清单常驻、全文按需加载」上。
...
> ⚠️ **划重点防踩坑**：Codex 的技能目录是 `.agents/skills`（仓库级）和 `$HOME/.agents/skills`（用户级），**不是** `~/.codex/skills/`。`~/.codex/` 是放 `config.toml` 这类**配置**的地方（第 05 节会用到），别把技能文件夹建到那儿去——建错了 Codex 扫不到，你会以为 Skill 坏了。这跟第 11 篇 `AGENTS.md` 那套发现链是两码事，别混。
...
> 💡 一句话总结：造用 `$skill-creator`（默认纯指令）、装用 `$skill-installer`、禁用在 `~/.codex/config.toml` 写 `[[skills.config]]`、关隐式触发在技能目录放 `agents/openai.yaml` 设 `allow_implicit_invocation: false`——**注意配置在 `.codex`、技能文件在 `.agents`，两个目录别搞混**。
...
几条最容易踩的坑，临走再钉一遍：**目录是 `.agents` 不是 `.codex`、字段没有 `trigger`、显式调用是 `$` 不是 `@`**——这三点凡是跟你看过的某些老教程对不上的，**以官方为准**。
