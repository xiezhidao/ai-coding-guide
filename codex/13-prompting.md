...
- 用 `/goal`（目标模式）把验收标准钉成「不达标不收工」的官方玩法
...
flowchart TD
    A[一个需求] --> B{大不大?}
    B -->|大活| C[/plan 出方案<br/>或拆成小步]
    B -->|小活| D[四件套<br/>目标·范围·约束·验证]
    C --> D
    D --> E[Codex 代理循环<br/>想→做→看]
    E --> F{有没有可跑的验证?}
    F -->|有| G[它自己跑检查<br/>没过接着改]
    F -->|没有| H[凭感觉收工<br/>漏洞你来逮]
    G --> I[审 diff → 批准落地]
    H --> I
```
