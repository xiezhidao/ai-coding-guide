...
拿一个真实项目对比两周：同一套代码、同样的活儿，一周用官方 Sonnet，一周用 DeepSeek。下面这张表是实际用下来最真实的感受，不是参数表抄的。
...
所以更实用的用法是**混着来**（后面第 05 节讲怎么混）：糙活给便宜模型，硬骨头切回 Claude。
...
![一个请求从 Claude Code 经环境变量改写后寄到 DeepSeek 又原路返回的时序图](assets/05-third-party-models/request-flow-deepseek@2x.png)
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | `haiku` 别名 / 后台任务的模型 | 轻量：快速问答、后台杂活 |
...
还有一个控制「思考深度」的变量 `CLAUDE_CODE_EFFORT_LEVEL`，官方支持 `low` / `medium` / `high` / `xhigh` / `max`（具体哪些档可用取决于模型）。想让模型多想一会儿、结果更稳，可以调高；想省 token、要快，就调低。
