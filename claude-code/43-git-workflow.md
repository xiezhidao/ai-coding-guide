...
```mermaid
flowchart LR
    A["改完代码<br/>（它已经帮你写好）"] --> B["看 diff<br/>它总结：改了啥/有没有夹带脏东西"]
    B --> C["写 commit<br/>它照历史风格+CLAUDE.md 写，你过目"]
    C --> D["开 PR<br/>它用 gh pr create，自动绑会话"]
    D --> E{"要推到远端？"}
    E -->|"push / force"| F["你亲手在终端敲<br/>（deny 已焊死，它碰不了）"]
    E -->|"还没好，继续改"| B
    style F fill:#5a1e1e,stroke:#e06c6c,color:#fff
