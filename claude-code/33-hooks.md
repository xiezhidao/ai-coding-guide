...
**类比：家里贴的几条自动化规则（「当……就……」）。** 你给智能家居设过那种规则吧——「**当**有人开门，**就**自动亮灯」「**当**电热水壶烧到 100 度，**就**自动断电」「**当**我离家，**就**关掉所有插座」。这些规则有个共同点：**条件一旦满足，动作必然发生，不需要谁去判断、去决定、去记得**。Hook 就是给 Claude Code 装的这种规则——你定义「**当**某个事件发生，**就**跑这段命令」，剩下的它自动执行，雷打不动。

这里要把一个**最关键的区别**钉死，它直接决定了你什么时候该用 Hook。在第 30 篇（功能怎么选）里我们提过一句，这里展开讲透：

> CLAUDE.md 或 skill 中的「永远不要编辑 `.env`」之类的说明是请求，而不是保证。阻止编辑的 `PreToolUse` hook 是强制执行。

说白了：

- **写进 CLAUDE.md 的，是「请求」**——你拜托 Claude「记得做 X」，它大概率照办，但**可能漏、可能忘**，因为它每一步都在「自己判断」。
- **配成 Hook 的，是「保证」**——只要那个事件触发，动作**一定**执行，跟 Claude 想不想、记不记得没半点关系。
...
**类比：还是家里那条自动化规则——你得给它设个条件。** 「开门就亮灯」如果不加条件，那**任何一扇门**（卧室、厕所、冰箱门）一开全亮，鸡飞狗跳。你得指定「**只有大门**开了才亮灯」。`matcher` 干的就是这个「指定哪扇门」的活。
