...
| **装什么** | 指令和规则 | 它学到的经验和模式 |
| **典型内容** | 编码标准、工作流、项目架构 | 构建命令、调试心得、它发现的你的偏好 |
| **什么时候加载** | 每次会话，**全量加载** | 每次会话，但**只加载前 200 行或 25KB** |
| **范围** | 用户 / 项目 / 本地级 | 每个项目一份，同仓库的 worktree 共享 |
...
**类比：还是那张随手贴的便利贴。** 你跟一个干活久了的搭档配合，有些事你压根不用反复交代——他自己会在心里（或便利贴上）记下来：「这项目构建得跑 `make build` 不是 `npm build`」「上次那个偶发 bug 是时区没设对」。下次遇到类似的，他自己就想起来了。**自动记忆就是 Claude 的这本便利贴：你不用写一个字，它边干边记。**
...
专门拎出来说一句，因为不少老教程、老视频里都教过：**在输入框用 `#` 开头打一句话，快速追加记忆。** [18] 里提过一嘴，这里说透。

**结论：`#` 是早期的老做法，新版本里这套交互已经变了。以官方现在的做法为准。** 官方现在的记忆体系里，正经入口是上面讲的那套——**让 Claude「记住 xxx」（它存进自动记忆）、或用 `/memory` 自己编辑**。

所以别再纠结 `#` 怎么打了，记住现在这两条路就行：

| 你想干的事 | 现在的正确做法 |
|-----------|--------------|
| 让它记一条心得 / 偏好 | 直接说「记住 xxx」「以后都用 xxx」→ 存进**自动记忆** |
| 想明确写进 CLAUDE.md | 把话说全：「把 xxx **加进 CLAUDE.md**」 |
| 查 / 改 / 删已记的东西 | 敲 `/memory`，掀开文件直接编辑 |

这里有个**新手极易踩的混淆点，务必分清**：你光说一句「记住 xxx」，它**默认多半存进自动记忆**（它自己那本笔记），**不一定进 CLAUDE.md**。这俩是两套文件（回看第 01 节那张表）。**想让某条规矩进 CLAUDE.md 这张「正式守则」，就得把「加进 CLAUDE.md」这句话说全**，别只说「记住」。

这里很容易栽一次：跟它说「记住依赖只用 pnpm」，以为进了 CLAUDE.md 全队都能共享，结果它存进了本机的自动记忆——**队友拉下来根本看不到**。搞清楚就明白了：要全队共享的死规矩，必须明说「加进 CLAUDE.md」（那个进 git）；只给自己、让它自己积累的，才走自动记忆。**一句话之差，共享范围天差地别。**

> 💡 一句话总结：`#` 是过时老做法别用了；现在让它记就说「记住 xxx」（进自动记忆）、要全队共享就说「加进 CLAUDE.md」（进 git）、查改删用 `/memory`——**「记住」默认进它自己的笔记，不等于进 CLAUDE.md，想共享得说全**。
