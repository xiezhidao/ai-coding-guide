...
先说清楚一件事：这几十个里，**绝大多数是「内置命令」**（行为写死在 CLI 里，按下去执行固定逻辑），还有一小撮在文档里标着 **Skill**——那是「捆绑 Skill」，本质是给 Claude 的一份提示，让它用自己的工具去编排完成（比如 `/code-review`、`/debug`）。**两者你都用 `/` 加名字调，用起来没区别**，差别只在底层怎么实现——这个细节第 26 篇讲 Skill 时拆过，这里知道有这回事即可。
...
这四组我没给全（官方那张总表有几十行，要查全的去 `references/official-docs-zh/commands.md`），但**覆盖了你日常九成的场景**。剩下的，临时想干啥，打个 `/` 加几个字母筛一下，比背一张表实在得多。
...
## 04 给命令加料：frontmatter 和 `$ARGUMENTS` 传参

上一节那个 `/review` 已经能用了，但它有个明显的局限：**内容写死，不能传参**。你没法告诉它「就审 `src/auth.ts` 这一个文件」——它每次都审全部改动。要让命令灵活起来，得加两样东西：**frontmatter（前置配置）** 和 **参数占位符**。
...
> **自定义斜杠命令，已经被官方合并进了 Skill 体系。** 你的 `.claude/commands/` 文件照样能用，但 Skill 是「升级版」——多了能带配套文件、能由 Claude 自动触发、平时几乎不占上下文这几样本事。
...
说白了，**你这一篇学的「自定义斜杠命令」，就是 Skill 体系最轻量的那个入口**。先用它把「一键提示」这件事玩顺，需要更多本事了，平滑升级到 Skill 就行——你写的 `.claude/commands/` 文件一行都不用改，照样能用。
