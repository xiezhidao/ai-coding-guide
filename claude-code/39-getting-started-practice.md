...
> 📚 **系列导航**：上一篇 [38 插件参考手册] 教你把自己那套配置打成一个能发出去的包。这一篇彻底换挡——**前面三十八篇全在「一件件拆功能」，这一篇第一次把它们串成一条完整的实战路径**：拿一个真实的小需求，从打开项目、写 CLAUDE.md、提问、审查改动到验证交付，一口气走完。学过的招，这回拧成一股绳用。

很多人刚学完前面几十篇，对着终端愣半天，憋出一句：

> 「道理我全懂了——CLAUDE.md、权限、检查点、怎么提问……可我现在到底**该先敲哪个**？」

把整趟流程打一仗看一遍，多半就一句话：「原来是这么个顺序。」
...
```mermaid
flowchart LR
    A["① 开工<br/>进目录·启动·定规矩"] --> B["② 探索<br/>让它先看懂代码"]
    B --> C["③ 规划<br/>出方案·你点头"]
    C --> D["④ 动手<br/>改代码·审 diff"]
    D --> E["⑤ 验证<br/>亲手跑一遍"]
    E --> F["⑥ 交付<br/>提交进库"]
```

这张图把一趟实战画成一条单向流水线：六个环节依次接棒，前一棒的产出正好是后一棒的输入——**「看懂了什么」喂给规划，「打算怎么改」喂给动手，「改了什么」喂给验证**。中间任何一环空掉，链条就断在那儿。

**类比：接力赛的交接棒。** 一场接力赛跑得好不好，不全看每个人跑得多快，**更看棒交得稳不稳**——上一棒没把棒递到位，下一棒跑得再猛也是白搭。这六步就是六个交接棒的人：探索把「我看懂了什么」交给规划，规划把「我打算怎么改」交给动手，动手把「我改了什么」交给验证。**任何一棒掉了棒，整趟都得返工**——新手翻车，九成是某个交接没做，最典型的就是「没探索就直接让它动手」，棒还没拿稳就开跑。
