...
- 团队的四个关键件（leader、队友、共享任务列表、信箱）各干什么
...
![Agent teams 团队结构：工头分活、队友并行、任务板协调、信箱互通](assets/29-agent-teams/team-structure@2x.png)
...
**类比：施工现场墙上那块工程进度看板。** 你不用挨个工位去问「干到哪了」，抬头看看看板就行——哪道工序在进行、哪个卡住等你拍板、哪个已经验收完了，一目了然。`claude agents` 就是 Claude Code 的这块看板：每个会话占一行，行首一个图标用颜色和动画告诉你它的状态。
