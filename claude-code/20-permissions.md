...
> 📚 **系列导航**：上一篇 [19 上下文管理] 教你怎么管好 Claude 的「工作台」、别让它把记忆塞爆。这一篇换个维度——管的不是「它记得多少」，而是「它敢动多少」：从一行命令都要问你，到全自动放飞，权限这根缰绳到底攥多紧，你说了算。
...
两个官方设计的「保险」让你稍微安心：一是即便在这个模式下，**`rm -rf /` 和 `rm -rf ~` 这种删根目录 / 家目录的操作仍会提示**，当作防手滑的断路器；二是在 Linux / macOS 上**以 root 或 `sudo` 身份是直接拒绝启动**这个模式的。但别指望保险——**核心还是「只在删了也不心疼的隔离环境里开」**。
