...
> 一个推荐的选择：个人项目一律走 Max 订阅登录，省心、不用盯余额；只有往 GitHub Actions 里塞自动化任务时，才单独搞一个 API key（具体到 CI 的玩法第 44 篇再讲）。**日常开发别碰 API key，纯属给自己找扣费焦虑。**
...
![身份验证优先级栈：Claude Code 从顶往下找第一个有值的层](assets/04-api-config/auth-priority-stack@2x.png)
...
**类比：派活儿挑人。** Opus 是组里**最强的资深工程师**——脑子好、推理深，但慢、贵；Sonnet 是**主力干将**——日常编码又快又稳，性价比高；Haiku 是**跑腿小弟**——简单活儿秒回、最省。难题派 Opus，日常派 Sonnet，杂活派 Haiku。
