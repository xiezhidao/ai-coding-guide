...
值得先点一句：这五块里，**沙箱、网络、模型是你十有八九会真动手调的**；devcontainer 偏团队场景，终端配置偏个人手感。所以如果你时间紧，先把第 02、04、06 三节吃透，剩下的当字典随用随查。
...
```mermaid
flowchart LR
    A["不开隔离<br/>主机裸跑"] --> B["自带沙箱<br/>只圈 Bash"]
    B --> C["devcontainer<br/>圈整个环境"]
    C --> D["容器 / 虚拟机<br/>圈到操作系统"]
    D --> E["Claude Code on the web<br/>云端托管隔离"]
```
...
最后一块，**模型配置**——直接关系到「贵不贵、聪不聪明」。第 04、06 篇讲过怎么接模型、怎么计费，这一节只讲「怎么按任务挑、怎么换、怎么省」。
...
| **`sonnet`** | 最新 Sonnet | 日常编码，绝大多数活儿的主力 |
...
设了之后，成员通过 `/model`、`--model`、环境变量都切不到列表外的模型。**对控团队成本特别实在。** 在一个预算敏感的项目里这么干，把默认锁 Sonnet、Opus 直接从可选列表里拿掉，**当月的 token 消耗肉眼可见地降了下来**，该用 Opus 的硬活儿单独开会话临时切就行。
