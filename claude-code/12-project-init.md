...
> 📚 **系列导航**：上一篇 [11 网页版与云端](./11-web-and-cloud.md) 带你跳出终端，在浏览器和云端环境里用上了 Claude Code。这一篇回到本地、回到一个最该养成的习惯——进新项目第一件事，敲 `/init`，让 Claude 自己扫一遍代码库、把项目说明书写出来。
...
这一步是关键，**一定要 `cd` 进项目的根目录再启动**，不能在桌面或家目录裸启。第 07 篇就强调过：你在哪启动，Claude 就把哪儿当工作区、读哪儿的文件。在家目录跑 `/init`，它扫的是你一堆乱七八糟的个人文件，扫了个寂寞。
...
![/init 五步初始化流程](assets/12-project-init/init-flow@2x.png)
