...
- 五条核心法则：给验证手段、先探索再编码、把话说具体、CLAUDE.md 写精、及时纠偏
...
- **非交互模式跑脚本**：`claude -p "你的提示"` 不开交互会话直接出结果，这是把 Claude 塞进 CI、pre-commit hook、批处理脚本的入口。比如大规模迁移，可以写个循环对几千个文件逐个 `claude -p`，配 `--allowedTools` 把它无人值守时能干的事框死。
...
| 「它写出来的东西方向就错了」 | **先探索再编码** | 不熟 / 改多文件，先进 Plan Mode；一句话能说清的直接干 |
