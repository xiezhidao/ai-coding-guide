...
> 📚 **系列导航**：上一篇 [34 CLI 参考手册：命令与全部标志] 帮你把「启动 Claude 之前」能拨的开关查全了。这一篇换到「会话已经开起来」之后——你跟 Claude 一来一回的过程里，手里其实攥着一整块「控制台」：怎么按一下就让它换工作模式、怎么让它先出方案别急着动手、怎么花钱换更快的响应、怎么用 Vim 手势编辑你的输入。**第 20 篇讲过权限那根缰绳，这一篇讲你手上的全套操纵杆。**
...
推荐的实操习惯，跟第 20 篇说的一致——**绝大多数时候只在默认三档里 `Shift+Tab` 手动切**：接陌生项目先切 `plan` 让它通读出方案，信得过方向了切 `acceptEdits` 放它改，改完想严一点再切回 `default`。`bypassPermissions` 只在隔离容器里用启动参数开，别挂进日常循环——这道红线第 20 篇画过，不重复了。
...
| 退回对话上一个点 | `Esc` `Esc`（空框时） | 打开回退菜单，细节见第 37 篇 |
