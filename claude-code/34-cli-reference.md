...
这里要分清两个标志，**别用错**：`--append-system-prompt` 是「**在默认提示后面追加**」，Claude 还保留它原本那套编码助手的本事和安全指令，你只是补一条额外要求；`--system-prompt` 则是「**整个替换掉**」默认提示——连工具用法、安全约束全没了，得你自己对剩下的一切负责。**九成场景你要的是 `append`（追加），不是 `system-prompt`（替换）**；只有当 Claude 那套「编码助手」的默认身份压根不适合你的任务时，才用替换。
