...
> 📚 **系列导航**：上一篇 [22 MCP] 教你给 Claude 接上外部工具，让它能查数据库、连 GitHub。这一篇换个思路——不是给它「加工具」，而是教你把活儿**外包出去**：子代理（Subagent），一个带独立上下文、独立权限、独立人设的专项助手，干完只把结论交回来。
...
回想第 19 篇讲的上下文——Claude 的「工作台」是有限的，你跟它聊得越久、读的文件越多，台面越挤，挤满了它就开始忘事、变笨。**有些活会疯狂占台面，但产出的中间垃圾你压根不会再看第二眼。**
...
![主对话与子代理的上下文隔离：脏活在子代理房间里干，只有结论回到主线](assets/23-subagents/subagent-isolation@2x.png)
...
| `permissionMode` | 否 | 它的权限模式 | 取值跟第 20 篇那六种一致（`plan`/`acceptEdits` 等） |
